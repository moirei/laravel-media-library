...
class ShareController extends Controller
{
    ...
    public function get(SharedContent $shareable)
    {
        ...
        return view('media-library::content')->with([
            ...
            'can_download' => ($shareable->max_downloads === 0) || ($shareable->max_downloads < $shareable->downloads),
    ...
    public function download(SharedContent $shareable)
    {
        ...
        if (Api::isFolder($shareable->shareable)) {
            $name = $shareable->name;
            if (is_null($name)) $name = data_get($shareable, 'shareable.name');
    ...
    public function upload(Request $request, SharedContent $shareable)
    {
        ...
        if (!$upload->checkType($allowed_upload_types))
            abort(422, __('Forbidden file format') . " $upload->filename");

        if (!$upload->checkSize($max_upload_size))
    ...
    public function validateAccess(SharedContent $shareable, array $credentials)
    {
        ...
        $code = $key =  data_get($credentials, 'code');
        $auth = false;
